...
def render_book_reader_inspector(prefix: str = "book_reader") -> None:
    ...
    default_book = next((book["pdf_name"] for book in books if "הקוד השנתי" in book["pdf_name"]), books[0]["pdf_name"])
    book_names = [book["pdf_name"] for book in books]
    book_key = f"{prefix}_book"
    page_key = f"{prefix}_page"
    chosen_book = st.selectbox("בחר ספר", book_names, index=book_names.index(default_book) if default_book in book_names else 0, key=book_key)
